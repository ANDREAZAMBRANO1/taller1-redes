...
echo "timestamp,cpu_percent,mem_total_percent,mem_real_percent,response_ms,apache_procs,connect" > $LOG_FILE
...
monitor_loop() {
	...
	CPU_INT=$(echo $CPU | cut -d'.' -f1)
        ...
        # Memoria
        # Memoria total usada (incluye buffers y caché)
	MEM_TOTAL=$(free | awk '/Mem:/ {printf "%.1f", $3/$2 * 100}' | tr ',' '.')
	MEM_TOTAL_INT=$(echo $MEM_TOTAL | cut -d'.' -f1)

	MEM_REAL=$(free | awk '/Mem:/ {printf "%.1f", ($3-$6)/$2 * 100}' | tr ',' '.')
	MEM_REAL_INT=$(echo $MEM_REAL | cut -d'.' -f1)
        ...
        # Guardar en CSV
        echo "$(date '+%Y-%m-%d %H:%M:%S'),$CPU,$MEM_TOTAL,$MEM_REAL,$RESPONSE_TIME,$APACHE_PROCS,$CONNECTIONS,$LOAD_AVG" >> $LOG_FILE
        ...
        echo -e "💾 ${BLUE}MEMORIA TOTAL:${NC} $(print_progress_bar $MEM_TOTAL_INT) ${YELLOW}$MEM_TOTAL%${NC}"
	echo -e "💾 ${BLUE}MEMORIA REAL:${NC}  $(print_progress_bar $MEM_REAL_INT) ${YELLOW}$MEM_REAL%${NC}"
